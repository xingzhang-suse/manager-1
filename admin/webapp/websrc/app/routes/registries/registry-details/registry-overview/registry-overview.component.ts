...
export class RegistryOverviewComponent implements OnChanges {
  ...
  static sortByVulnerabilities(a: Image, b: Image): number {
    ...
    return a.critical + a.high + a.medium > b.critical + b.high + b.medium ? -1 : 1;
  }
    ...
    if (!this.isAllView && changes.registryDetails) {
      ...
      const topFive = sortedRegistryDetails.slice(0, 5).map(image => {
        return {
          ...
          tag: image.tag,
          vulnerabilities: { critical: image.critical, high: image.high, medium: image.medium },
        };
      });
      const otherVulnerabilities = sortedRegistryDetails
        .slice(5, sortedRegistryDetails.length)
        .reduce((sum, { critical, high, medium }) => sum + critical + high + medium, 0);
      const topSix = [
        ...topFive.map(i => {
          return {
            repository: i.repository,
            tag: i.tag,
            vulnerabilities: i.vulnerabilities.critical + i.vulnerabilities.high + i.vulnerabilities.medium,
          };
        }),
        {
    ...
    if (this.isAllView && changes.summary4AllView) {
      ...
      const topSix = topImages.map(i => {
        return {
          ...
          vulnerabilities: i.critical + i.high + i.medium,
        };
      });
